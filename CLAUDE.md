...
- **Status emojis** (v1.7.0): tab + banner title share one vocab — SessionStart→⏸️ startup, UserPromptSubmit→⏳ running, Notification→🔔 needs-you, Stop→👀 your-turn. `cc_status_emoji` also reserves `success`→✅ / `failure`→❌ for a future turn-outcome feature (Stop can't tell success from failure yet). Colored circles stay reserved for `/color` identity only. Format: `<status> <color> <name>`.
