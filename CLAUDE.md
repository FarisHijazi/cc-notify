...
- **Terminal-tab rename** (v1.7.0): cc-notify-bg fires `open -g "<scheme>://farishijazi.cc-notify-focus/rename?pids=…&name=<emoji title>"` (background → no foreground steal). The extension runs `workbench.action.terminal.renameWithArg {name}` **only when Claude's terminal is the activeTerminal** (renameWithArg targets the active terminal; this avoids stealing focus or renaming the wrong tab). Native tab *color* is unsettable via API (confirmed) → emoji-only. Verified: tab → "🟠 cc-notify". See @LESSONS.md #15.
