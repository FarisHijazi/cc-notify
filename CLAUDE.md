...
- **Status emojis** (v1.7.0): tab + banner title share one vocab — SessionStart→⏸️ startup, UserPromptSubmit→⏳ running, Notification→🔔 needs-you, Stop→ outcome. Colored circles stay reserved for `/color` identity only. Format: `<status> <color> <name>`.
