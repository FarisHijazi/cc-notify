...
- **Status emojis** (v1.7.0): tab + banner title share one vocab — SessionStart→⏸️ startup, UserPromptSubmit→⏳ running, Notification→🔐 permission / ❓ question-input / 🔔 generic (split by `notification_type` + message text; raw types logged to `notiftypes.log` to refine), Stop→ outcome. Colored circles stay reserved for `/color` identity only. Format: `<status> <color> <name>`.
...
- **Turn outcome on Stop** (v1.7.0): the global `~/.claude/CLAUDE.md` instructs Claude to end every message with a trailing `✅`/`❌`/`💬`. `cc_last_status_token` reads the last text-bearing assistant message from the transcript and returns that emoji → Stop status = ✅ task complete / ❌ failed / 💬 other, falling back to 👀 "your turn" when no token is present (older messages / instruction not followed). Banner subtitle follows: "Task complete" / "Task failed" / "Turn complete". NOTE: this depends on an out-of-repo instruction in the user's global CLAUDE.md.
