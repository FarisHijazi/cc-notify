...
- **Per-session color emoji** (v1.7.0): reads `agentColor` (set by `/color`, stored as `"agentColor":"…"` in the transcript JSONL — same source `gsd-statusline.js` uses) from `transcript_path`, maps red/orange/yellow/green/blue/purple/pink/cyan → 🔴🟠🟡🟢🔵🟣🩷🩵, prefixes the banner subtitle. There is **no Claude Code session name** exposed to hooks (researched) — only `session_id`/`cwd`/`transcript_path`.
